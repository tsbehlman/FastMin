...
const SKIP_SPACE_BEFORE_CHARS =	new Set( Buffer.from( "\t\n\r !\"$%')*+,-/:;=>]^{|}~" ) );
const SKIP_SPACE_AFTER_CHARS =	new Set( Buffer.from( "!\"'()*+,:;=>[]{}~" ) );
...
CSSMinifier.prototype.processChar = function( c, output, outputIndex ) {
	...
	case SPACE_BEFORE_CHAR:
		if( this.state === SPACE_BEFORE_CHAR && !SKIP_SPACE_BEFORE_CHARS.has( c ) ) {
	...
	case REGULAR:
		...
		else if( SKIP_SPACE_AFTER_CHARS.has( c ) ) {
			this.state = SPACE_AFTER_CHAR;
		}
		else if( c <= 32 ) {					// 32 === " "
